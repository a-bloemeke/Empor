...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-20 pb-20 -mt-6">
      ...
      {/* ── Stats strip ───────────────────────────────────────────────── */}
      {!isLoggedIn && (
        <section className="rounded-xl border border-primary/30 bg-card px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="flex-1 space-y-1">
            <p className="text-sm font-semibold">{t("registerNoticeTitle")}</p>
            <p className="text-xs text-muted-foreground leading-relaxed">{t("registerNoticeDesc")}</p>
          </div>
          <Link
            href="/register"
